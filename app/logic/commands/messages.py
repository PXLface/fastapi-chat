...
class CreateChatCommandHandler(CommandHandler[CreateChatCommand, Chat]):
    chat_reposirory: BaseChatRepository
    async def handle(self, command: CreateChatCommand) -> Chat:
        if await self.chat_reposirory.check_chat_exists_by_title(command.title):
            raise ChatWithThatTitleAlreadyExistsException(command.title)

        title = Title(value=command.title)
        
        new_chat = Chat.create_chat(title=title)
        await self.chat_reposirory.add_chat(new_chat)

        await self.chat_reposirory.add_chat
